...
ER_path     = [base_path, 'images/ER-allTissue/'];
PR_path     = [base_path, 'images/PR-allTissue/'];
HER2_path   = [base_path, 'images/HER2-allTissue/'];
...
for i = 1:nBioMarkers
    ...
    for j = 1:nImages(1);
        ...
        if sum(Im_info.cohort_num == 1:4)
            Im_info.patches = process_image(Im_info, tilesize, nCellThreshold);
        end   
...
for i = 1:nImages(1);
    ...
    for j = 1:numel(Images{1, i}.patches)
        ...
        for k = 1:nBioMarkers
         ...
         imwrite(patch, [base_path, 'patches/cohort_', ...
